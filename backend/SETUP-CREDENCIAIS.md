...
Dá para rodar **todo o projeto usando o mock da Meta** incluído — não é obrigatório ter um
número/app real. Este guia é para quem quiser ir além e testar contra a API real, e para
explicar como obter a chave da OpenAI com custo mínimo.
...
2. **Crie um Project dedicado** para este projeto (menu de Projects no topo). Isso isola os
...
4. Em **API keys**, gere uma chave **dentro do Project** criado e coloque em `OPENAI_API_KEY`
   no seu `.env`.
5. Use o modelo **`gpt-4o-mini`** — é barato e mais que suficiente para este projeto. Para
   alguns milhares de mensagens, o custo fica em centavos de dólar.
...
> desenvolvimento. Por isso o passo do **System User token** abaixo é importante para uso prolongado.
...
6. **Token que não expira (recomendado para uso prolongado)** — crie um *System User*:
...
Use o mock incluído: `POST http://localhost:8001/simulate/inbound` injeta uma mensagem assinada
como se viesse da Meta, e `POST http://localhost:8001/{phoneNumberId}/messages` recebe os
seus envios. Veja o [README](README.md#-quickstart).
